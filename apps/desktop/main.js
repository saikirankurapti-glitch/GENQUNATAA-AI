...
function createMainWindow() {
  mainWindow = new BrowserWindow({ width: 1280, height: 820, minWidth: 960, minHeight: 640, title: 'GenQuantaa AI', backgroundColor: '#0b0d0c', webPreferences: { preload: path.join(__dirname, 'preload.js'), contextIsolation: true, nodeIntegration: false, sandbox: true } });
  mainWindow.loadURL(WEB_URL);
}

function createOverlay() {
  const { width } = screen.getPrimaryDisplay().workAreaSize;
  overlayWindow = new BrowserWindow({ width: 430, height: 280, x: Math.max(16, width - 450), y: 24, frame: false, transparent: true, resizable: false, alwaysOnTop: true, skipTaskbar: true, title: 'GenQuantaa AI Copilot', webPreferences: { preload: path.join(__dirname, 'preload.js'), contextIsolation: true, nodeIntegration: false, sandbox: true } });
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('detect-meeting-provider', (_event, meetingUrl) => { const provider = detectMeetingProvider(meetingUrl); return provider ? { id: provider.id, name: provider.name } : null; });
  ipcMain.handle('meeting-providers', () => PROVIDERS.map(({ id, name }) => ({ id, name })));
  ipcMain.handle('start-meeting-monitor', async (_event, payload) => { try { return await meetingOrchestrator.start(payload || {}); } catch (error) { return { active: false, status: 'error', error: error instanceof Error ? error.message : 'Meeting orchestration failed' }; } });
