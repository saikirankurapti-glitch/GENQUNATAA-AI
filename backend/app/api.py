...
async def session_analytics(db: AsyncSession = Depends(get_db)) -> dict:
    ...
    for session in sessions:
        ...
        rows.append({
            ...
            "question_count": int(questions or 0), "note_count": int(notes or 0),
...
async def chat(payload: ChatRequest, db: AsyncSession = Depends(get_db)) -> ChatResponse:
    ...
    if session_id is None:
        session = SessionRecord(); db.add(session); await db.flush(); session_id = session.id
...
async def analyze_visual(file: UploadFile) -> dict:
    allowed = {"image/png", "image/jpeg", "image/webp"}
    if file.content_type not in allowed:
        raise HTTPException(status_code=415, detail="Only PNG, JPEG, and WebP screenshots are supported")
    ...
    try:
        analysis = await ai.analyze_visual(image, file.content_type)
    except RuntimeError as exc:
...
async def analyze_question(session_id: UUID, payload: dict, db: AsyncSession = Depends(get_db)) -> dict:
    if not await db.get(SessionRecord, session_id): raise HTTPException(status_code=404, detail="Session not found")
    transcript = str(payload.get("transcript", "")).strip()
    if not transcript: raise HTTPException(status_code=400, detail="transcript is required")
    try: return await interview_intelligence.analyze_question(db, session_id, transcript)
    except Exception as exc:
        await db.rollback(); raise HTTPException(status_code=502, detail=f"Interview analysis failed: {exc}") from exc
