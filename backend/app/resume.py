...
KNOWN_SKILLS = {
    "python", "sql", "azure", "azure data factory", "azure databricks", "databricks",
    "adls", "azure data lake", "spark", "pyspark", "aws", "gcp", "snowflake", "dbt",
    "airflow", "kafka", "java", "scala", "terraform", "docker", "kubernetes", "git",
    "power bi", "machine learning", "tensorflow", "pandas", "numpy",
}
...
async def upload_resume(file: UploadFile = File(...), db: AsyncSession = Depends(get_db)) -> ResumeOut:
...
async def latest_resume(db: AsyncSession = Depends(get_db)) -> ResumeOut | None:
    result = await db.execute(select(ResumeProfile, Document).join(Document, Document.id == ResumeProfile.document_id).order_by(Document.created_at.desc()).limit(1))
    row = result.first()
    if not row:
        return None
