...
from .db_models import ResumeProfile, SessionRecord, UserRecord
...
strengths:array of strings'''
        fallback = {
            "role_summary": f"Interview strategy for {target_role}.",
            "skill_gap_matrix": [{"skill": "Core role fundamentals", "importance": "high", "candidate_level": "partial", "gap": "Validate depth with practical questions", "evidence": skills}],
            "priority_topics": ["fundamentals", "implementation", "troubleshooting", "system design"],
            "interview_roadmap": [
                {"stage": "Fundamentals", "objective": "Validate core knowledge", "topics": ["fundamentals"], "question_weight": 0.25, "recommended_questions": []},
                {"stage": "Implementation", "objective": "Test hands-on depth", "topics": ["implementation", "debugging"], "question_weight": 0.35, "recommended_questions": []},
                {"stage": "Architecture", "objective": "Evaluate design trade-offs", "topics": ["system design"], "question_weight": 0.25, "recommended_questions": []},
                {"stage": "Behavioral", "objective": "Evaluate ownership and communication", "topics": ["projects", "incidents"], "question_weight": 0.15, "recommended_questions": []},
            ],
            "preparation_plan": [{"topic": "Role fundamentals", "action": "Practice concise explanations and production examples", "priority": "high", "estimated_minutes": 45}],
            "likely_question_types": ["technical", "system_design", "project", "behavioral"],
            "readiness_score": 0.5,
            "risks": ["Job-description-specific gaps require review"],
            "strengths": [skills],
        }
        result = fallback
        if self.ai.api_key:
            ...
            response = await genai.Client(api_key=self.ai.api_key).aio.models.generate_content(
                model=self.ai.model, contents=prompt,
...
async def analyze(payload: dict[str, Any], db: AsyncSession = Depends(get_db), user: UserRecord = Depends(current_user)) -> dict[str, Any]:
    target_role = str(payload.get("target_role", "Data Engineer")).strip()
    job_description = str(payload.get("job_description", "")).strip()
    if not job_description:
        raise HTTPException(status_code=400, detail="job_description is required")
    if len(job_description) > 30000:
        raise HTTPException(status_code=400, detail="job_description is too long")
...
async def session_strategy(session_id: UUID, payload: dict[str, Any], db: AsyncSession = Depends(get_db), user: UserRecord = Depends(current_user)) -> dict[str, Any]:
    ...
    if not job_description:
        raise HTTPException(status_code=400, detail="job_description is required")
    return await engine.analyze(db, user, str(payload.get("target_role", "Data Engineer")), job_description)
